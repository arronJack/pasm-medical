...
| `DEPT_ADMIN` | `department` / `staffId` | 本科室 | — |
| `SUPER_ADMIN` | — | 全院 | 对接设置 + 资料库 |
...
>
> ⚠️ **如实标注**：医护与科室管理员目前**共享同一档能力**，只差科室绑定 —— 二者真正的
> 差异要等「科室实体 + 本科室资源」（P1-2）才有实质内容。现在硬造一个差异（例如让科室
> 管理员管资料库）反而是错的：资料库当前没有科室维度，给了写权限就等于一个科室改全院。
...
> 「脚本坏了」，看不出是「口径 / 埋点错了」。
>
> ★ **反例对照要一次改坏多处**：本轮同时改坏 3 处（患者侧收窄 / 科室收窄 / 会话隔离），
> 得到**恰好 6 条转红**、其余 45 条保持绿；失败详情本身就是泄漏证据
> （例如 `{'ref': 'demo-patient-002', 'chronic': '2 型糖尿病'}`）。
...
# ★ 三端联调（认知服务 + Spring Boot + 前端契约，51 项；含 10 项越权/非法输入反例）
...
| MCP stdio sidecar（`pasm_medical/mcp/`） | ✅ 已建并验证（selftest 24 项 + 三端 e2e 51 项）；复用 `MedicalService` 门面，不启 HTTP 零端口污染 |
| Spring Boot 业务层 | ✅ 已编译、已启动、三端联调全过；JPA 持久化（患者 / 就诊 / 审计 / **对接设置**）+ 真实管理接口（`/api/admin/*`、`/api/patient`、`/api/patient/encounters`、`/api/patient/encounter/{id}`、`/api/admin/config`、`/api/auth/me`）；**后台限医护侧（配置与资料库仅超管）+ 按身份收窄数据范围** |
