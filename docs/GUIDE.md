...
> **后台授权分两层**（缺一层都不够）：
> ① **URL 级** —— `/api/admin/**` 只给医护侧角色，患者令牌一律 403；其中
> 「对接设置」（决定患者数据会不会出网）与「资料库」（全院的答案依据）**仅超管**。
> ② **资源级** —— 医护 / 科室管理员只看**本科室**的数据（顶栏常驻显示范围）；
> 带 `ref` 的接口一律按身份收窄。只做 ① 就等于"能进这个接口"＝"能看全院所有患者的过敏史"。
...
> （把实现改回旧逻辑时该断言**精确转红**：`before=0 after=0`，其余 66 项保持绿）。
...
| 来源 | 存在哪 | 谁能改 | 返回时怎么标 |
|---|---|---|---|
| **机构资料库**（指南 / 规则 / 术语 / 科室手册） | 业务库 `knowledge_docs`（权威）+ 认知侧检索副本 | 后台「资料库」页（`/api/admin/kb`，**限超级管理员**） | `sources[].kind = "doc"` |
...
| 角色 | 账号 | 密码 | 科室 / 绑定 | 落地页 | 数据范围 |
|---|---|---|---|---|---|
| 超级管理员 | `staff` | `123456` | — | `/#/admin` | 全院；唯一能改对接设置与资料库 |
| 科室管理员 | `deptadmin` | `123456` | 发热门诊 | `/#/admin` | 本科室 |
| 医护人员 | `doctor` | `123456` | 发热门诊 | `/#/admin` | 本科室 |
| 患者 | `patient` | `123456` | `demo-patient-001` | `/#/consult` | 本人 |

演示数据含**两位**患者（`demo-patient-001` 发热门诊 / 心内科；`demo-patient-002` **骨科**）——
第二位是刻意加的：只有一位患者时「科室过滤」与「全院不过滤」结果完全一样，范围规则测不出差异。
数据由 `bootstrap/DemoDataSeeder` 灌入。

登录页 `http://127.0.0.1:5173/#/login`。**`npm run dev` 时页面上直接显示这 4 个账号，点一下自动填入**
...
- **患者令牌换 `ref`** 读他人档案 / 就诊列表 / 以他人 `ref` 发起问诊，必须 **403**；
- 别的患者用同一 `sessionKey` 回答 → 必须**查不到会话**（会话按患者隔离）；
- `--medical.auth.demo-login-enabled=false` 时，这些账号必须**登不进去**。

生产必须接医院统一身份（OIDC/OAuth2），见 `config/TokenService.java` 的注释与 §5.2。

#### 4.4.2 身份与数据范围（P1-1）

★ **身份 → 业务实体的映射是权限的地基**。在这之前登录只返回一个角色字符串，
账号 `patient` 与业务实体 `demo-patient-001` 之间没有任何绑定 —— 于是
「这个患者只能看自己的记录」这条最基础的规则**根本无法表达**，
`/api/patient/**` 只能退化成「登录了就能看」（换个 `ref` 就读到别人的病历）。

现在的模型（`config/Identity.java` + `config/Role.java`）：

| 角色 | 关键字段 | 数据范围 | 可写 |
|---|---|---|---|
| `PATIENT` | `patientRef` | **本人**（`ref` 由身份决定，请求参数只是「申请」） | — |
| `DOCTOR` | `department` / `staffId` | 本科室 | — |
| `DEPT_ADMIN` | `department` / `staffId` | 本科室 | **本科室资料库**（P1-2） |
| `SUPER_ADMIN` | — | 全院 | 对接设置 + 资料库（全院通用 + 各科室） |

**两条判定分工**（`SecurityConfig` 只能做第一条，第二条必须写在接口里）：

1. **URL 级**（`SecurityConfig`）：这类角色能不能调这个接口。
2. **资源级**（`IdentityContext.scopedRef`）：这个身份能不能访问那条 `ref`。
   所有带 `ref` / `patientRef` 的接口**一律先过它**；越权时 `403` 并**先记审计**
   （越权尝试是数据泄露的前兆，也是 IDOR 探测唯一的证据来源）。

> ★ **顺带修掉的两个真问题（都在本轮实测中发现）**：
> ① **会话未按患者隔离** —— 会话键原先恒为 `"current"`（全租户共一条），
>    患者 B 回答 `"current"` 会改到患者 A 的进行中会话，采集到的事实还会写进 A 的记忆。
>    现在会话键 = `患者ref + ":" + sessionKey`（`MedicalService._session_key`），
>    `answer` / `finish` 必须带 `patient_ref`，否则如实返回失败而不是退回全局会话。
> ② **登录审计把令牌当成了操作者** —— 原先写的是 `a.setActor(r[0])`，而 `r[0]` 是令牌串本身，
>    后果是审计里查不出「是谁登的」，且**访问令牌被明文落进审计表**。现在记账号名。
>
> ✅ **P1-2（2026-09-23）起，医护与科室管理员已拉开真实能力差异** —— 资料库引入**科室维度**
> （`KnowledgeDoc.department`，"空 = 全院通用"），科室管理员成为第一个真正能写资料库的角色，
> 但**只能管本科室**资料：
>   - 新建强制归到本科室（请求里带别的科室也落不到别科）；
>   - 更新 / 上架下架 / 删除**跨科室或全院通用（空）**资料一律 `403`
>     （资源级判定 `IdentityContext.canManageDepartment`）；
>   - 列表只返回本科室资料；改系统配置仍 `403`（仅超管，决定患者数据是否出网）。
>
> 医护（`DOCTOR`）对资料库仍 **403** —— 临床读资料走问答接口，不需要资料库后台。
> 这条差异之所以**现在**才落：必须先有科室维度，否则给写权限等于让一个科室改掉全院所有
> 科室的答案依据（这正是本轮之前一直被文档警告、宁可不给的原因）。
...
**67 项全过**代表三端真的接上了。其中 20 项是**反例**（故意构造的越权/非法输入）：
就诊归属不符必须 404（挡 IDOR）、非法枚举值必须 400、患者令牌访问后台必须 403、
**患者换 `ref` 必须 403**、**别的患者回答同一会话必须查不到**、
`demo-login-enabled=false` 时写死的账号必须登不进去、
**科室管理员动跨科室/全院通用资料必须 403**。

另有 **12 项是「改回旧实现就必须转红」的断言**（它们盯的是「不报错但结果错」的缺陷）：
...
| 新增资料后提问必须命中 `doc` 来源 | 不再合并机构资料 → **拒答** → 转红 |
| 患者换 `ref` 读他人记录必须 403（3 条） | 去掉患者侧收窄 → 返回 **200 + 他人数据** → 转红 |
| 别的患者回答同一会话必须查不到 | 会话键不再带患者 → **回答成功** → 转红 |
| 发热门诊医护看不到骨科那位患者 | 不再按科室收窄 → **全院名单** → 转红 |
| 科室管理员新建资料必须**落在本科室**（请求带别科也不行） | 不强制本科室 → 写成骨科资料 → 转红 |
| 科室管理员列表必须**只含本科室** | 不做科室过滤 → 混入全院/别科 → 转红 |
| 科室管理员动**全院通用 / 别科**资料必须 403（4 条） | 去掉科室收窄 → 返回 **200** → 转红 |

> ★ 断言都写成「**None 安全 + 命中计数**」：字段缺失或没记上时给出一条**清晰的失败**，
> 而不是在断言表达式里抛 `TypeError`/`KeyError` 把整个套件带崩 —— 崩溃只会让人以为
> 「脚本坏了」，看不出是「口径 / 埋点错了」。
>
> ★ **反例对照要一次改坏多处**：P1-1 轮同时改坏 3 处（患者侧收窄 / 科室收窄 / 会话隔离），
> 得到**恰好 6 条转红**、其余 45 条保持绿；P1-2 轮同样一次改坏 3 处（本科室强制 /
> 列表科室过滤 / 资源级科室收窄），得到**恰好 6 条转红**、其余 61 条保持绿
> （红的是「新建强制本科室 / 列表只含本科室 / 动全院通用或别科→403」这 6 条）。
> 失败详情本身就是泄漏证据（例如 `{'ref': 'demo-patient-002', 'chronic': '2 型糖尿病'}`）。
> 只改一处容易「恰好测到」，一次改多处才能证明断言各自盯着各自的分支。
...
# ★ 三端联调（认知服务 + Spring Boot + 前端契约，67 项；含 20 项越权/非法输入反例）
...
配置漂移判定分别故意改坏 → `tools/e2e_stack.py` 的对应 3 项**确实转红**（其余 64 项仍绿），
还原后回到 67/67。P1-2 的科室授权也照此办理：一次改坏 3 处 → 恰好 6 条转红、其余 61 条保持绿。
只报"全绿"而不报"改坏能被抓到"，等于没验证。
...
| MCP stdio sidecar（`pasm_medical/mcp/`） | ✅ 已建并验证（selftest 24 项 + 三端 e2e 67 项）；复用 `MedicalService` 门面，不启 HTTP 零端口污染 |
| Spring Boot 业务层 | ✅ 已编译、已启动、三端联调全过；JPA 持久化（患者 / 就诊 / 审计 / **对接设置**）+ 真实管理接口（`/api/admin/*`、`/api/patient`、`/api/patient/encounters`、`/api/patient/encounter/{id}`、`/api/admin/config`、`/api/auth/me`）；**后台限医护侧（配置仅超管、资料库按科室授权）+ 按身份收窄数据范围** |
| 前端 | ✅ 三栏工作台 + 医护后台，**全部接通真实后端**（登录 / 问诊 / 检验单 / 反馈 / 患者档案 / 历史就诊详情 / 后台患者·统计·审计·对接设置），构建通过、零 TS 错误 |
| 影像 | ❌ 不做分析，只归档 + 转交 |
| 合规 | ❌ 未做医疗器械分类界定；规则表**未经临床药师复核** |

> ✅ **已解决（P1-1，2026-09-23）**：带 `ref` 的接口**不再只校验「已认证」**，而是先过
> `IdentityContext.scopedRef` —— 「令牌 → 该患者 ref」的映射由 `TokenService` 提供
> （`patient` 账号 ↔ `demo-patient-001`，不再是"对不上"）。患者令牌**不传 `ref` 即本人、
> 传别人的 `ref` 直接 403**；`e2e_stack` 有 3 条反例盯着这件事（查档案 / 查就诊列表 / 发起问诊）。
> 就诊详情（`/api/patient/encounter/{id}`）另有**归属校验** —— 它的 id 是自增整数，
> 不校验就等于把全院病历开放出去，属于必须当场堵住的越权。
>
> ⚠️ **有意保留的边界（不是漏洞）**：**医护侧**读个体档案（`/api/patient?ref=`）**不做科室收窄**
> —— 临床本就需要跨患者查档，且 `ref` 是不透明 ID（难猜）。科室收窄只作用于**聚合视图**
> （后台患者列表 / 审计 / 统计）。若将来要收紧，只需改 `scopedRef` 这一处，
> 且 `e2e_stack` 里已有一条断言把"维持现状"锁住，改动会立刻转红提醒。
