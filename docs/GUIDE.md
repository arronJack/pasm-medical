...
| MCP stdio sidecar（`pasm_medical/mcp/`） | ✅ 已建并验证（selftest 19 项 + e2e 15 项）；复用 `MedicalService` 门面，不启 HTTP 零端口污染 |
| Spring Boot 业务层 | ✅ 已编译、已启动、三端联调全过；JPA 持久化（患者 / 就诊 / 审计实体 + Repository + Service）、登录/问诊/检验单审计埋点、真实管理接口（`/api/admin/*`、`/api/patient`、`/api/patient/encounters`） |
| 前端 | ✅ 三栏工作台 + 医护后台，**全部接通真实后端**（登录 / 问诊 / 检验单 / 反馈 / 患者档案 / 历史就诊 / 后台患者·统计·审计），构建通过、零 TS 错误 |
| 影像 | ❌ 不做分析，只归档 + 转交 |
