...
│                                                # AdminController（后台，限医护侧）
│                                                # AdminKbController（资料库，P1-2 按科室授权）
├── web/                    # Vue 3 + TS —— 诊疗工作台
...
|---|---|---|---|---|---|
| **超级管理员** | `staff` | `123456` | — | `/#/admin` | **全院**；唯一能改「对接设置」，资料库可管全院通用 + 各科室 |
| **科室管理员** | `deptadmin` | `123456` | 发热门诊 | `/#/admin` | 本科室；**可写本科室资料库**（跨科室 / 全院通用资料 → 403） |
| **医护人员** | `doctor` | `123456` | 发热门诊 | `/#/admin` | 本科室；资料库仍 403（临床读资料走问答接口） |
...
- **两层授权**：① URL 级 —— `/api/admin/**` 只给医护侧，改配置**仅超管**、
  资料库按科室授权（科室管理员限本科室）；② **资源级** —— 带 `ref` 的接口一律按身份收窄，
  患者换个 `ref` 读别人档案/就诊/发起问诊全部 **403**（拿 `patient` 令牌试
  `?ref=demo-patient-002` 就是这条）；资料库的写/停用/删除再按资料科室收窄（P1-2）。
...
python tools/e2e_stack.py                # 三端联调，67 项（四角色全覆盖，需先 mvn package）
...
| **授权与越权防护** | ✅ **两层判定**：URL 级（后台只给医护侧；改配置**仅超管**、资料库按科室授权）+ **资源级**（`ref` 按身份收窄 —— 患者换 `ref` 读他人档案 / 就诊 / 发起问诊全部 **403**；资料库写/停用/删除按资料科室收窄 —— 科室管理员动跨科室/全院通用资料 **403**）；就诊详情**归属校验**（挡 IDOR）；对接设置**服务端白名单校验** |
| **角色与数据范围** | ✅ 四角色（患者 / 医护 / 科室管理员 / 超管）+ **身份 → 业务实体绑定**（`patientRef` / 科室 / 工号）；`/api/auth/me` 下发身份与可读范围，后台顶栏常驻显示「数据范围：本科室：发热门诊 / 全院」。✅ **P1-2：医护与科室管理员已拉开真实差异**（资料库引入**科室维度**，科室管理员只能管本科室资料 —— 新建强制本科室、跨科室/全院通用→403、列表仅本科室）；⚠ 科室维度统计未实现（`/stats` 仍是全院口径，响应显式带 `scope`/`scopeNote`，**不假装**是本科室数字） |
...
| **资料库（回答依据）** | ✅ 后台可**新增 / 编辑 / 上下架 / 删除**；**P1-2 按科室授权**（`KnowledgeDoc.department`，空 = 全院通用）：科室管理员只能管本科室资料，医护仍 403，超管管全院；业务库是权威、认知侧只放"当前生效资料"的检索副本（全量同步）；**下架的资料不再作为任何回答的依据**。带依据问答走医疗侧的**相关性闸门**（命中词必须落在标题或标签上），没依据就拒答 —— 闸门与资料库都有正反断言盯着 |
| 三端联调 | ✅ `tools/e2e_stack.py` **67 项全过**（四角色全覆盖：患者 / 医护 / 科室管理员 / 超管；含越权/非法输入/会话隔离反例 + 资料库科室授权反例 + 个体读维持现状锁定；**12 项 e2e_stack 判据已做反向验证**：P1-1 6 项（安全判据 / 统计口径 / 审计埋点 / 资料库链路 / 身份数据范围 / 会话隔离）+ P1-2 6 项（本科室强制 / 列表科室过滤 / 资源级科室收窄）—— 一次改坏多处，恰好这 12 条转红、其余保持绿） |
...
**已验证的规模**：模块自检 **88 项** + MCP selftest **24 项** + 认知端到端 **17 项** + 学习端到端 **14 项**（P0.5）+ 反例对照 **5 项** + 学习反例对照 **2 项**（P0.5）+ 三端联调 **67 项**（四角色全覆盖，全部通过）。
