...
#    演示账号（仅 dev，密码均 123456）：staff（超管·全院）、deptadmin（科室管理员·发热门诊）、
#    doctor（医护·发热门诊）、patient（患者·绑定 demo-patient-001）
...
|---|---|---|---|---|---|
| **超级管理员** | `staff` | `123456` | — | `/#/admin` | **全院**；唯一能改「对接设置」与「资料库」的角色 |
| **科室管理员** | `deptadmin` | `123456` | 发热门诊 | `/#/admin` | 本科室 |
| **医护人员** | `doctor` | `123456` | 发热门诊 | `/#/admin` | 本科室 |
| **患者** | `patient` | `123456` | 绑定 `demo-patient-001` | `/#/consult` | **本人** |

入口 `http://127.0.0.1:5173`（跑法见上面「快速开始」第 2、3 步）。演示数据含 **2 位患者**：
`demo-patient-001`（女 46 · 青霉素过敏 · 高血压 3 级 · 发热门诊 + 心内科就诊）与
`demo-patient-002`（男 58 · 2 型糖尿病 · **骨科**就诊）。放第二位是**刻意的** ——
只有一位患者时「本科室过滤」与「全院不过滤」结果完全一样，范围规则就测不出差异。

- **身份由账号决定**，登录页不再让你选角色：让用户先选一份再登录，等于允许
  「我声称我是医护」，而真正生效的永远是服务端返回的角色。
- **两层授权**：① URL 级 —— `/api/admin/**` 只给医护侧，改配置与资料库**仅超管**；
  ② **资源级** —— 带 `ref` 的接口一律按身份收窄，患者换个 `ref` 读别人档案/就诊/发起问诊
  全部 **403**（拿 `patient` 令牌试 `?ref=demo-patient-002` 就是这条）。
- 开发模式登录页**直接提示**这 4 个账号，点一下自动填入；`npm run build` 产物里
  **不含**该提示、也不含 `123456` 字样（可 grep 产物验证）。
...
python tools/e2e_stack.py                # 三端联调，60 项（四角色全覆盖，需先 mvn package）
...
| **授权与越权防护** | ✅ **两层判定**：URL 级（后台只给医护侧；改配置 / 资料库**仅超管**）+ **资源级**（`ref` 按身份收窄 —— 患者换 `ref` 读他人档案 / 就诊 / 发起问诊全部 **403**）；就诊详情**归属校验**（挡 IDOR）；对接设置**服务端白名单校验** |
| **角色与数据范围** | ✅ 四角色（患者 / 医护 / 科室管理员 / 超管）+ **身份 → 业务实体绑定**（`patientRef` / 科室 / 工号）；`/api/auth/me` 下发身份与可读范围，后台顶栏常驻显示「数据范围：本科室：发热门诊 / 全院」。⚠ doctor 与 deptadmin **当前有意同权**（只差科室绑定），真正差异递延 P1-2（科室实体 + 本科室资源）；⚠ 科室维度统计未实现（`/stats` 仍是全院口径，响应显式带 `scope`/`scopeNote`，**不假装**是本科室数字） |
...
| 三端联调 | ✅ `tools/e2e_stack.py` **60 项全过**（四角色全覆盖：患者 / 医护 / 科室管理员 / 超管；含越权/非法输入/会话隔离反例 + 资料库写操作反例 + 个体读维持现状锁定；**17 项判据已做反向验证**：安全判据 + 统计口径/审计埋点 + 资料库链路 + 身份/数据范围/会话隔离 + 个体读维持现状锁定） |
| **受控学习 / 反馈作用面（P0.5）** | ✅ 医学动作后验落地（`pasm_medical/learning.py` 自包含 Beta-Bernoulli，绝不训练聊天动作池）：医生 `adopt/reject` 改接 `POST /api/feedback`，**只影响分诊候选排序、不碰安全规则**；否决后候选排序可观测变化、predict 概率 `<0.5`、非法 `decision` 返回 **400**；聊天动作池前缀被后验拒绝。验收 `tools/e2e_medical_learning.py`（14 项）+ `tools/falsify_medical_learning.py`（2 反例），已接入 `run_checks` ②b / ③b |
| 影像归档 | 未开始（P5，**只做归档与转交，不做分析**） |
| 中医知识库 | 未开始（见 `docs/PLAN.md` §6） |

**已验证的规模**：模块自检 **88 项** + MCP selftest **24 项** + 认知端到端 **17 项** + 学习端到端 **14 项**（P0.5）+ 反例对照 **5 项** + 学习反例对照 **2 项**（P0.5）+ 三端联调 **60 项**（四角色全覆盖，全部通过）。
