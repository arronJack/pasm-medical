...
python tools/e2e_stack.py                # 三端联调，51 项（需先 mvn package）
...
| **角色与数据范围** | ✅ 四角色（患者 / 医护 / 科室管理员 / 超管）+ **身份 → 业务实体绑定**（`patientRef` / 科室 / 工号）；`/api/auth/me` 下发身份与可读范围，后台顶栏常驻显示「数据范围：本科室：发热门诊 / 全院」。⚠ 科室维度统计未实现（`/stats` 仍是全院口径，响应显式带 `scope`/`scopeNote`，**不假装**是本科室数字） |
...
| 三端联调 | ✅ `tools/e2e_stack.py` **51 项全过**（含 10 项越权/非法输入反例；**14 项判据已做反向验证**：3 个安全判据 + 3 个统计口径/审计埋点 + 2 个资料库链路 + 6 个身份/数据范围/会话隔离） |
...
**已验证的规模**：模块自检 **88 项** + MCP selftest **24 项** + 认知端到端 **17 项** + 学习端到端 **14 项**（P0.5）+ 反例对照 **5 项** + 学习反例对照 **2 项**（P0.5）+ 三端联调 **51 项**（全部通过）。
